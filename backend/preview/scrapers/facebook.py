...
class FacebookScraper(BaseScraper):
    ...
    def get_apify_actor_id(self):
        ...
        return os.environ.get("APIFY_FACEBOOK_ACTOR_ID", "")

    def normalize_apify_data(self, apify_data: dict) -> dict:
        """
        Normalize Apify actor output for Facebook to our standard preview format.
        ...
        return {
            ...
            "embed_html": None,  # Facebook doesn't provide embed HTML via standard meta tags
    ...
    def fetch_preview(self) -> dict:
        ...
        title = self.get_meta(html, "og:title") or self.get_meta(html, "title")
        description = self.get_meta(html, "og:description") or self.get_meta(
            html, "description"
        )
        image = self.get_meta(html, "og:image")
        author = self.get_meta(html, "og:site_name")
        author_url = self.get_meta(html, "og:url")
        # Note: og:video is a URL to the video file, not embed HTML
        # For proper Facebook video embedding, you'd need to use Facebook's embed API
        embed_html = None
        return self.normalize_fields(
            {
                ...
                "video_url": None,  # Video URL would require additional API calls
                "thumbnail_url": None,
            }
        )